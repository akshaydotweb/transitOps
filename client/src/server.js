...
app.get('/trains/:id/seats', (req, res) => {
  const trainId = req.params.id;

  const query = `
    CALL GetAvailableSeats(?, @seatsAvailable);
    SELECT @seatsAvailable AS seatsAvailable;
  `;

  pool.query(query, [trainId], (err, results) => {
    if (err) {
      console.error('Error executing stored procedure:', err);
      return res.status(500).json({ error: 'Database error', message: err.message });
    }

    const seatsAvailable = results[1][0].seatsAvailable;
    res.json({ seatsAvailable });
  });
});

// Get revenue for a specific train
app.get('/trains/:id/revenue', (req, res) => {
  const trainId = req.params.id;

  const query = `
    SELECT CalculateRevenue(?) AS revenue;
  `;
...
app.post('/bookings', (req, res) => {
  const { train_id, user_id, seats_booked } = req.body;

  // Basic validation
  if (!train_id || !user_id || seats_booked == null) {
    return res.status(400).json({ error: 'All fields are required' });
  }

  const insertQuery = `
    INSERT INTO bookings 
      (train_id, user_id, seats_booked)
    VALUES 
      (?, ?, ?)
  `;

  pool.execute(insertQuery, [train_id, user_id, seats_booked], (err, results) => {
    if (err) {
